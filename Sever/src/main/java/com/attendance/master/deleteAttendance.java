...
@WebServlet(name = "deleteAttendance", value = "/master/deleteAttendance")
public class deleteAttendance extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		try{
			...
			int column = conn.createStatement().executeUpdate("DELETE FROM `attendance` WHERE `startTime` = '" + time + "';");
			
			conn.createStatement().executeUpdate("DELETE FROM `attendance_log` WHERE `attendanceTime` = '" + time + "';");
			
			if(column != 0){
				ParameterUtil.setStatus(js,Config.HTTP_OK,"删除成功！");
			}else{
				ParameterUtil.setStatus(js,Config.SEVER_ERROR,"删除失败！");
			}
		}catch (Exception e) {
